defmodule FibonacciRpcClient do
  def wait_for_messages(_channel, correlation_id) do
    receive do
      {:basic_deliver, payload, %{correlation_id: _correlation_id}} ->
        IO.puts("get response #{payload}")
        {n, _} = Integer.parse(payload)
        n
    end
    # FibonacciRpcClient.wait_for_messages()
  end
  def call(n) do
...
response = FibonacciRpcClient.call(5)
